...
function enhanceSidebar(){
  ...
  if(!sidebar.querySelector('.ux-sidebar-toggle')){
    ...
    const btn=document.createElement('button');btn.type='button';btn.className='btn ux-sidebar-toggle';btn.textContent='Afficher sources, risques et pièces';btn.setAttribute('aria-expanded','false');
    btn.addEventListener('click',()=>{const open=sidebar.classList.toggle('ux-details-open');btn.textContent=open?'Masquer sources, risques et pièces':'Afficher sources, risques et pièces';btn.setAttribute('aria-expanded',String(open))});
...
function buildWorkbar(){
  ...
  bar.id='uxWorkbar';bar.className='ux-workbar';bar.setAttribute('aria-label','Navigation et réglages du dossier');
  const stepper=document.createElement('div');stepper.className='ux-stepper';
  stepper.innerHTML='<button type="button" class="ux-step" id="uxPrevCase" aria-label="Dossier précédent">←</button><span class="ux-count" id="uxCaseCount"></span><button type="button" class="ux-step" id="uxNextCase" aria-label="Dossier suivant">→</button>';
