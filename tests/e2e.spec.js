...
test('memo exposes professional-control topics',async({page})=>{
  await clean(page);await page.getByRole('button',{name:'Mémo avancé'}).click();await expect(page.getByText('Grand livre → décompte',{exact:true})).toBeVisible();await expect(page.getByText('Importation, acquisition et DTe',{exact:true})).toBeVisible();await expect(page.getByText('Personnes liées et compensations',{exact:true})).toBeVisible();await expect(page.getByText('Concordance annuelle et rectification',{exact:true})).toBeVisible()
});
