...
const LEVEL_PLAN_STORAGE='tva_avance_v1_state';
...
function patchControls(){
  ...
  if(guided){guided.textContent='Rubriques utiles';guided.title='Afficher uniquement les rubriques utiles au dossier'}
  ...
  if(blocks[0]){const hint=document.createElement('span');hint.className='ux-mode-hint';const active=blocks[0].querySelector('[data-mode].active')?.dataset.mode||'learn';hint.textContent=modeHelp[active]||'';blocks[0].appendChild(hint)}
}
function patchAccessibleNames(){const qualification=$('#qualificationSelect');if(qualification&&!qualification.getAttribute('aria-label'))qualification.setAttribute('aria-label','Qualification TVA — choisir une réponse')}
function observeQualification(){const host=$('#qualification');if(!host||host.dataset.uxObserved)return;host.dataset.uxObserved='1';new MutationObserver(()=>requestAnimationFrame(patchAccessibleNames)).observe(host,{childList:true,subtree:true});patchAccessibleNames()}
function changeCase(delta){const select=$('#caseSelect');if(!select)return;const i=Math.max(0,Math.min(select.options.length-1,select.selectedIndex+delta));if(i===select.selectedIndex)return;select.selectedIndex=i;select.dispatchEvent(new Event('change',{bubbles:true}))}
function syncStepper(){const select=$('#caseSelect'),prev=$('#uxPrevCase'),next=$('#uxNextCase'),count=$('#uxCaseCount');if(!select)return;if(prev)prev.disabled=select.selectedIndex<=0;if(next)next.disabled=select.selectedIndex>=select.options.length-1;if(count)count.textContent=`${select.selectedIndex+1} / ${select.options.length}`}

function safePlanState(){try{return JSON.parse(localStorage.getItem(LEVEL_PLAN_STORAGE)||'{}')}catch{return{}}}
...
function ensureLevelPlan(){
  ...
  layer.innerHTML=`<div class="ux-plan-backdrop" data-plan-close></div><section class="ux-plan-dialog" role="dialog" aria-modal="true" aria-labelledby="uxLevelPlanTitle" tabindex="-1"><header class="ux-plan-head"><div><span class="ux-plan-eyebrow">Méthode effective · Niveau 2</span><h2 id="uxLevelPlanTitle">Plan du niveau</h2><p id="uxLevelPlanSummary"></p></div><button type="button" class="ux-plan-close" data-plan-close aria-label="Fermer le plan du niveau">×</button></header><div class="ux-plan-body" id="uxLevelPlanBody"></div><footer class="ux-plan-foot"><button type="button" class="btn" id="uxPlanFinal">Évaluation finale</button><button type="button" class="btn" data-plan-close>Fermer</button></footer></section>`;
  document.body.appendChild(layer);
  layer.addEventListener('click',e=>{
    if(e.target.matches('[data-plan-close]'))closeLevelPlan();
    const card=e.target.closest('[data-plan-case-index]');
    if(card){const select=$('#caseSelect'),index=Number(card.dataset.planCaseIndex);if(select&&Number.isInteger(index)){select.selectedIndex=index;select.dispatchEvent(new Event('change',{bubbles:true}))}closeLevelPlan();setTimeout(()=>$('#uxWorkbar')?.scrollIntoView({block:'start'}),0)}
  });
  $('#uxPlanFinal')?.addEventListener('click',()=>{closeLevelPlan();setTimeout(()=>{const target=$('#finalEvaluation');target?.scrollIntoView({behavior:'smooth',block:'start'});$('#startFinal')?.focus({preventScroll:true})},0)});
...
function renderLevelPlan(){
  ensureLevelPlan();const state=safePlanState(),body=$('#uxLevelPlanBody'),summary=$('#uxLevelPlanSummary'),select=$('#caseSelect');if(!body)return;
  ...
  if(summary)summary.textContent=`${mastered}/18 maîtrisés · choisissez un dossier pour reprendre directement.`;
  body.innerHTML=LEVEL_PLAN_GROUPS.map(group=>{
    const cases=group.ids.map(id=>CASES.find(c=>c.id===id)).filter(Boolean);const done=cases.filter(c=>statuses[c.id].key==='mastered').length;
...
function openLevelPlan(){ensureLevelPlan();renderLevelPlan();const layer=$('#uxLevelPlanLayer');if(!layer)return;planReturnFocus=document.activeElement;layer.hidden=false;document.body.classList.add('ux-plan-opened');layer.querySelector('.ux-plan-close')?.focus()}
...
function buildWorkbar(){
  ...
  const bar=document.createElement('section');bar.id='uxWorkbar';bar.className='ux-workbar';bar.setAttribute('aria-label','Navigation et réglages du dossier');
  const stepper=document.createElement('div');stepper.className='ux-stepper';stepper.innerHTML='<button type="button" class="ux-step" id="uxPrevCase" aria-label="Dossier précédent">←</button><span class="ux-count" id="uxCaseCount"></span><button type="button" class="ux-plan-open" id="uxLevelPlanOpen" aria-haspopup="dialog">Plan</button><button type="button" class="ux-step" id="uxNextCase" aria-label="Dossier suivant">→</button>';
