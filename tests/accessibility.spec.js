...
test('desktop initial Level 2 has no serious or critical automated WCAG violations',async({page})=>{
  await page.goto('/');
  await expect(page.locator('#uxWorkbar')).toBeVisible();
  await scan(page);
});

test('Level 2 plan dialog has no serious or critical automated WCAG violations',async({page})=>{
  await page.goto('/');
