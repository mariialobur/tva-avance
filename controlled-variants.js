...
const variants=[
  {
    ...
    options:['Déclarer la contre-prestation et l’impôt au ch. 383','Traiter automatiquement la facture comme une facture suisse avec IP ordinaire','Déduire le montant au ch. 221','Ignorer l’opération puisqu’aucune TVA ne figure sur la facture'],
    correct:0,
  ...
  },
  {
    ...
    options:['Oui, toute facture d’un fournisseur étranger va toujours au ch. 383','Non; dans cette variante, la TVA suisse facturée est traitée comme impôt préalable ordinaire et la même acquisition n’est pas redéclarée au ch. 383','Oui, mais uniquement au ch. 221','Non, et aucune déduction d’IP n’est jamais possible'],
    correct:1,
  ...
  },
  {
    ...
    options:['Oui, toute prestation étrangère est soumise au ch. 383','Non; l’impôt sur les acquisitions n’est pas dû si la prestation est exclue du champ de l’impôt ou exonérée dans les conditions applicables','Oui, mais au taux de 3,8 %','Oui, uniquement parce que le fournisseur n’est pas inscrit'],
    correct:1,
...
let activeId='AA',expanded=false;
...
function render(){
  ...
  if(currentCase()?.id!=='A'){expanded=false;return}
  if(currentMode()==='evaluate')return;
  const host=document.querySelector('#caseInfo');if(!host)return;
  const done=load().done||{};
  const active=variants.find(v=>v.id===activeId)||variants[0];
  const panel=document.createElement('section');panel.id='controlledVariantsA';panel.className=`controlled-variants ${expanded?'is-open':'is-collapsed'}`;
  const body=expanded?`<div class="cv-tabs" role="tablist" aria-label="Variantes du dossier A">${variants.map(v=>`<button type="button" role="tab" aria-selected="${v.id===active.id}" class="${v.id===active.id?'active':''} ${done[v.id]?'done':''}" data-cv-tab="${v.id}">${done[v.id]?'✓ ':''}${esc(v.short)}</button>`).join('')}</div>
