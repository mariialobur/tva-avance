...
const STORAGE_KEY='tva_avance_final_evaluation_v1';
...
function saveResult(r){
  ...
  if(previous?.passed&&(!r.passed||Number(previous.score)>Number(r.score)))return;
...
function startExam(){
  ...
  attempt={questions:chosen,startedAt:new Date().toISOString()};renderExam()
}
function setIsolation(active){
  [...document.body.children].forEach(node=>{if(node.id==='avanceExamLayer')return;active?node.setAttribute('inert',''):node.removeAttribute('inert')})
}
function renderExam(){
...
function submitExam(e){
  ...
  attempt.questions.forEach((q,i)=>{
    const selected=Number(form.get('q'+i)),chosen=q.options[selected],correct=Boolean(chosen?.correct);if(correct)score++;
    review.push({q:q.q,selected:chosen?.label||'',correctAnswer:q.options.find(o=>o.correct)?.label||'',correct,why:q.w,s:q.s,u:q.u})
  });
  const result={score,total:EXAM_SIZE,percent:Math.round(score/EXAM_SIZE*100),passed:score>=PASS_SCORE,date:new Date().toISOString(),review};
  saveResult(result);renderReview(result);launcher()
}
...
function openNameDialog(){
  if(!lastResult?.passed||masteredCount()!==CASES.length)return;
  let d=document.querySelector('#nameDialog');
  if(!d){
    d=document.createElement('dialog');d.id='nameDialog';d.className='name-dialog';
    d.innerHTML=`<form method="dialog"><div class="dialog-head"><h2>Attestation de parcours</h2><button value="cancel" class="icon-btn" aria-label="Fermer">×</button></div><p>Le nom est utilisé uniquement dans votre navigateur. L’identité n’est pas vérifiée et aucune donnée n’est envoyée par ce site.</p><label>Nom et prénom<input id="participantName" maxlength="90" autocomplete="name" required></label><div class="dialog-actions"><button value="cancel" class="btn">Annuler</button><button id="makeAttestation" value="default" class="btn primary">Générer l’attestation</button></div></form>`;
    document.body.appendChild(d);
    d.querySelector('#makeAttestation').onclick=e=>{e.preventDefault();const name=d.querySelector('#participantName').value.trim();if(name.length<2)return;d.close();renderAttestation(name)}
  }
  d.showModal();setTimeout(()=>d.querySelector('#participantName')?.focus(),0)
}
function renderAttestation(name){
  ...
  const themes=[
    ['International','Impôt sur les acquisitions, ch. 383, lieu de prestation et droit à déduction.'],
    ['Immobilier','Option, prestations exclues, affectation et corrections d’impôt préalable.'],
    ['Financements','Subventions, sponsoring, ch. 900 et réduction ch. 420.'],
    ['Groupe TVA','Traitement du groupe comme un seul assujetti et flux internes.'],
    ['Changements de méthode','TDFN ↔ méthode effective, valeurs résiduelles, ch. 410 et 415.'],
    ['Activités mixtes','Ventilation des statuts TVA et correction IP documentée.'],
    ['Contrôle & clôture','Concordance annuelle, rectification et dossier de preuves.']
  ];
  const wrap=document.createElement('div');wrap.id='attestationLayer';
  wrap.innerHTML=`<div class="attest-actions"><button class="btn" id="closeAttest">Fermer</button><button class="btn primary" id="printAttest">Imprimer / enregistrer en PDF</button></div>
  <section class="attest-page page-one"><div class="attest-kicker">Parcours pédagogique indépendant</div><h1>ATTESTATION DE PARCOURS</h1><h2>TVA suisse · pratique avancée</h2><p class="attest-name">${esc(name)}</p><p class="attest-main">a validé les <strong>${CASES.length} cas avancés en mode Évaluation à 100 %</strong> et a réussi l’auto-évaluation finale du parcours.</p><div class="attest-stats"><div><strong>${CASES.length} / ${CASES.length}</strong><span>cas validés</span></div><div><strong>${score} / ${EXAM_SIZE}</strong><span>évaluation finale</span></div><div><strong>${date}</strong><span>date du résultat</span></div></div><div class="attest-themes">${themes.map(t=>`<span>${esc(t[0])}</span>`).join('')}</div><p class="attest-url">${PROJECT_URL}</p><p class="attest-disclaimer">Cette attestation confirme uniquement l’achèvement de ce parcours d’entraînement indépendant et la réussite de son auto-évaluation. Elle ne constitue ni un diplôme, ni un titre professionnel, ni une certification reconnue ou accréditée. Le projet est indépendant et sans affiliation avec l’AFC/ESTV ou le SEFRI. Le nom est saisi par le participant; l’identité et le résultat ne sont pas vérifiables par un tiers auprès du site.</p></section>
  <section class="attest-page page-two"><h1>RELEVÉ DU PARCOURS</h1><p class="attest-sub">Thèmes travaillés dans le parcours TVA suisse avancé.</p><div class="attest-person"><strong>${esc(name)}</strong><span>${CASES.length} cas validés · résultat final ${score}/${EXAM_SIZE}</span></div><div class="attest-grid">${themes.map(t=>`<article><h3>${esc(t[0])}</h3><p>${esc(t[1])}</p></article>`).join('')}</div><div class="attest-reference"><strong>Référentiel pédagogique</strong><p>LTVA · OTVA · prototype du décompte et publications AFC. Les sources officielles et la pratique en vigueur restent déterminantes pour un dossier réel.</p><p>Sources principales contrôlées le 17.08.2026 · ${PROJECT_URL}</p></div><p class="attest-disclaimer">Ce relevé décrit les thèmes abordés. Il n’atteste pas de compétences professionnelles, d’un diplôme ou d’une certification reconnue. Il est généré localement à partir de la progression enregistrée dans le navigateur.</p></section>`;
...
window.addEventListener('avance-progress',launcher);
window.addEventListener('DOMContentLoaded',launcher);
